    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (Transfer $transfer) {
                return '<a href="' . route("transfer", $transfer->id) . '" class="btn btn-md btn-primary text-white font-weight-bold"> Manage </a>';
            })
            ->addColumn('email', function (Transfer $transfer){
                return $transfer->user->email ?? '';
            })
            ->addColumn('created_at', function (Transfer $transfer){
                return date('Y-m-d H:i:sa', strtotime($transfer->created_at));
            })
